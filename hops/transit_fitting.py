...
class TransitAndPolyFitting():
    ...
    def __init__(self, data,
                 ...
                 inclination, periastron, mid_time,
                 method='claret', precision=3, exp_time=0, time_factor=1,
                 stellar_logg=None, stellar_temperature=None, stellar_metallicity=None,
                 photometric_filter=None, stellar_model='ATLAS',
                 fit_first_order=False, fit_second_order=False,
                 fit_linear_ld=False, fit_quadratic_ld=False, fit_sqrt_ld=False,
                 fit_rp_over_rs=False, fit_fp_over_fs=False, fit_period=False, fit_sma_over_rs=False,
                 fit_eccentricity=False, fit_inclination=False, fit_periastron=False, fit_mid_time=False,
                 iterations=130000, walkers=200, burn=30000, counter=True, counter_window=False):

        self.data = data
        self.total_sets = len(self.data)

        if photometric_filter is not None:
            self.photometric_filter = photometric_filter

        if stellar_logg is not None:
            self.stellar_logg = stellar_logg

        if stellar_temperature is not None:
            self.stellar_temperature = stellar_temperature

        if stellar_metallicity is not None:
        ...
        self.fit_linear_ld = fit_linear_ld
        self.fit_quadratic_ld = fit_quadratic_ld
        self.fit_sqrt_ld = fit_sqrt_ld

        if self.fit_linear_ld:
            self.method = 'linear'
            if limb_darkening_coefficients is None:
                self.limb_darkening_coefficients = [0.5]

        elif self.fit_quadratic_ld:
            self.method = 'quad'
            if limb_darkening_coefficients is None:
                self.limb_darkening_coefficients = [0.5, 0.5]

        elif self.fit_sqrt_ld:
            self.method = 'sqrt'
            if limb_darkening_coefficients is None:
                self.limb_darkening_coefficients = [0.5, 0.5]
            ...
            if limb_darkening_coefficients is None:
                self.limb_darkening_coefficients = clablimb(self.stellar_logg, self.stellar_temperature,
                                                            self.stellar_metallicity, self.photometric_filter,
        ...
        self.fit_fp_over_fs = fit_fp_over_fs
        ...
        if self.fit_linear_ld or self.fit_quadratic_ld or self.fit_sqrt_ld:
        ...
        if self.method in ['claret', 'quad', 'sqrt']:
            ...
            if self.fit_quadratic_ld or self.fit_sqrt_ld:
                self.limits1.append(0.000001)
                self.limits2.append(0.999999)
            else:
                self.limits1.append(np.nan)
        ...
        for var in range(3 * self.total_sets + 4, len(self.names)):
    ...
    def transit_model(self, *model_variables):
        ...
        limb_darkening_coefficients = model_variables[3 * self.total_sets: 3 * self.total_sets + self.total_ldcs]

        transit_hr = transit(limb_darkening_coefficients, *model_variables[3 * self.total_sets + self.total_ldcs:],
                             time_array=self.data_time_hr, method=self.method, precision=self.precision)
