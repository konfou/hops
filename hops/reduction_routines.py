...
def reduction():
    ...
    for counter, science_file in enumerate(observation_files):
        ...
        fits[0].data = (data_frame - master_bias - fits[0].header[exposure_time_key] * master_dark) / master_flat
    ...
    if not exit_var.get():
        ...
        reduction_trash_directory = read_log('pipeline', 'reduction_trash_directory')
        trash = read_log('pipeline', 'trash')
        if not trash:
            list_to_remove = []
        else:
            list_to_remove = map(int, trash)
        ...
        ax.set_ylabel('sky (counts / pixel)')
        ...
        write_log('pipeline', map(str, list_to_remove), 'trash')